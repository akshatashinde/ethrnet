...
from django.shortcuts import (
    render, HttpResponse, render_to_response,
...
def inventory_detail(request, pk):
    ...
    last_month = IteamVariation.objects.filter(
        inventoryitem=inventory_item, purchased_at__lt=month)
    sdate = None
    ldate = None
    custom = None
    mnth = None
    ...
    if request.is_ajax():
        if request.method == "POST" and request.POST['action'] == 'start1':
            ...
            custom = IteamVariation.objects.filter(
                inventoryitem=inventory_item,
                purchased_at__range=(sdate, ldate)
            )
            print custom
        ...
        if request.method == "POST" and request.POST['action'] == 'start2':
            ...
            yearwise = IteamVariation.objects.filter(
                inventoryitem=inventory_item, purchased_at__year=year)
        ...
        if request.method == "POST" and request.POST['action'] == 'start3':
            ...
            monthwise = IteamVariation.objects.filter(
                inventoryitem=inventory_item, purchased_at__month=mnth)
        ...
        if request.method == "POST" and request.POST['action'] == 'start4':
            ...
            return HttpResponse(last_month_data,
...
def generate_item_excel(request, pk):
    ...
    if mnth != 0:
        items = IteamVariation.objects.filter(
            inventoryitem=inventory_item, purchased_at__month=mnth)
        print items, 1
    elif year != 0:
        items = IteamVariation.objects.filter(
        ...
        items = IteamVariation.objects.filter(
            inventoryitem=inventory_item, purchased_at__range=(sdate, ldate))
        print items, 3
    ...
    response[
        'Content-Disposition'] = 'attachment; filename=Inventory_details.xls'
    ...
    row_num = 0
    columns = ['Item_Code', 'Status', 'Quantity',
               'Price', 'Sale_price', 'purchased_at']
    ...
    for item in objectlist:
        row_num += 1
        row = [item['code'], item['status'], item['quantity'],
               item['price'], item['sale_price'], item['purchased_at']]
...
def piechart(request):
    ...
    ydata = [all_clients.filter(is_active=True).count(), all_clients.filter(
...
def connecntion_detail(request, pk):
    ...
    month = datetime.now() - timedelta(days=30)
    last_month = ConnectionHistory.objects.filter(
        client=clients, created_on__lt=month)
    sdate = None
    ldate = None
    custom = None
    mnth = None
    if request.is_ajax():
        if request.method == "POST" and request.POST['action'] == 'start1':
            ...
            request.session['sdate'] = sdate
            request.session['ldate'] = ldate
            custom = ConnectionHistory.objects.filter(
                client=clients, created_on__range=(sdate, ldate))
            print custom
            data = serializers.serialize("json", custom)
            return HttpResponse(data, content_type='application/json')
            # to_json = []
            ...
            # return
            # HttpResponse(response_data,mimetype_type='application/json')

        if request.method == "POST" and request.POST['action'] == 'start2':
            year = request.POST.get('currentdate')
            request.session['year'] = year
            yearwise = ConnectionHistory.objects.filter(
                client=clients, created_on__year=year)
            year_data = serializers.serialize("json", yearwise)
            return HttpResponse(year_data, content_type='application/json')
        ...
        if request.method == "POST" and request.POST['action'] == 'start3':
            ...
            request.session['mnth'] = mnth
            print mnth
            monthwise = ConnectionHistory.objects.filter(
                client=clients, created_on__month=mnth)
            print monthwise
            month_data = serializers.serialize("json", monthwise)
            return HttpResponse(month_data, content_type='application/json')
        if request.method == "POST" and request.POST['action'] == 'start4':
...
def generate_book_excel(request, pk):
    ...
    if mnth != 0:
        books = ConnectionHistory.objects.filter(
            client=clients, created_on__month=mnth)
        print books, 1
    elif year != 0:
        books = ConnectionHistory.objects.filter(
    ...
    # the name of the Excel file.
    response[
        'Content-Disposition'] = 'attachment; filename=Connection_History.xls'
    ...
    for book in objectlist:
        row_num += 1
        row = [book['client_id'], book['client'], book[
            'plan'], book['created_on'], book['expired_on']]
        for col_num in range(len(row)):
            worksheet.write(row_num, col_num, row[col_num])
