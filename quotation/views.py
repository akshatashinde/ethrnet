...
def createitem(request):
    ...
    return render(request, 'quotation/create.html',
                  {'user_form': user_form, 'item_form': item_form})
...
def createitem_formset(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and formset.is_valid():
            ...
            messages.error(
                request,
                "Properly fill information....entered another quotation no")
            return HttpResponseRedirect('')
    ...
    return render(request, 'quotation/create1.html',
    	{'user_form': user_form, 'formset': formset})
